    ...
    validate({
      body: z.strictObject({
        query: z
          .array(
            z.strictObject({
              dynamicId: z.coerce.number().positive(),
            ...
            await Promise.all(
              batch.map(async (updateReq) => {
                ...
                try {
                  ...
                  safeSetAttr(organizationNode.info, 'name', name);
                  safeSetAttr(organizationNode.info, 'color', color);
