...
import { writeNodeSnapshot } from './snapshotUtils';
...
module.exports = function (
  ...
  app.post('/api/v1/snapshot/nodes', async (req, res) => {
    ...
    try {
      const result = await writeNodeSnapshot();
      console.log(
        `snapshot : ${result.nodeCount} nodes out of ${result.objectCount} objects written to ${result.file} in ${result.durationMs}ms`
      );
