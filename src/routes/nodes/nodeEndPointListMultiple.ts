...
import type { EndPointNodeMultiple } from '../interface/EndPointNodeMultiple';
  ...
  app.post('/api/v1/node/endpoint_list_multiple', async (req, res, next) => {
    try {
      ...
      const promises = ids.map((id) =>
        getEndpointsInfo(spinalAPIMiddleware, profileId, id).then(
          (endpoints): EndPointNodeMultiple => ({
            dynamicId: id,
            endpoints: endpoints!,
          })
        )
