...
app.post('/api/v1/node/ticket_list_multiple', async (req, res, next) => {
  try {
      ...
      const promises = ids.map(id => getTicketListInfo(spinalAPIMiddleware, id).then(tickets => ({ dynamicId: id, tickets })));
      ...
      const finalResults = settledResults.map((result, index) => {
          if (result.status === 'fulfilled') {
              ...
              return {
                  dynamicId: ids[index],
                  error: result.reason?.message || result.reason || "Failed to get Tickets"
              };
          }
      });
      ...
      console.error(error);
      return res.status(400).send('An error occurred while fetching ticket list.');
  }
});
  };
