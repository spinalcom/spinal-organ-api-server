...
import {
  ...
  SPINAL_USER_ALPHA_GROUP_RELATION_TO_USER,
} from 'spinal-model-user-service';

export async function* getUserFromContextGen(
  userContext: SpinalContext,
  offset = 0
) {
  const userAlphaGroups = await getSpinalUserAlphaGroup(userContext);
  const userAlphaGroupsAndCount = userAlphaGroups.map((group) => ({
    group,
    count:
      group.children.PtrLst?.[SPINAL_USER_ALPHA_GROUP_RELATION_TO_USER]
