...
import { getStepFromProcessByStepId, getTicketLogs, getTicketInfo, getNodeFromTicket, getStepFromTicket, getProcessFromStep, getContextFromProcess, LOGS_EVENTS, SPINAL_TICKET_SERVICE_TICKET_TYPE } from "spinal-service-ticket";
...
async function getTicketDetails(spinalAPIMiddleware: ISpinalAPIMiddleware, profileId: string, ticketId: number, includeAttachedItems = true) {
...
async function getTicketLogDetails(ticketNode: SpinalNode, processNode: SpinalNode, contextNode: SpinalNode) {
...
async function formatEvent(log: SpinalLogTicketInterface, processNode: SpinalNode, contextNode: SpinalContext) {
	let texte = "";
	if (log.event == LOGS_EVENTS.creation) {
		...
		const [step1, step2] = await Promise.all([getStepFromProcessByStepId(contextNode, processNode, log.steps[0]), getStepFromProcessByStepId(contextNode, processNode, log.steps[1])]);
...
async function getTicketFiles(ticketNode: SpinalNode) {
	const files = await serviceDocumentation.getFileLinkedToNode(ticketNode);
...
async function getTicketNodeTree(ticketId: number, profileId: string, spinalAPIMiddleware: ISpinalAPIMiddleware) {
	const ticketNode: SpinalNode = await loadAndValidateNode(spinalAPIMiddleware, ticketId, profileId, SPINAL_TICKET_SERVICE_TICKET_TYPE);
	//Step
