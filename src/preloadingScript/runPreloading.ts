...
import { preloadingScript, type IPreloadingScript } from './preloadingScript';
import { findNodeSnapshot, runSnapshotPreloader } from './snapshotPreloader';
 ...
 */
export async function runPreloading(
  spinalAPIMiddleware: ISpinalAPIMiddleware,
  profileId: string,
  scriptOptions: IPreloadingScript
): Promise<PreloadStrategy> {
  const preload = spinalAPIMiddleware.config?.preload ?? config.preload;
  const workHours = formatWorkHours(preload);
  const startedDuringWorkHours = isWithinWorkHours(new Date(), preload);

  const snapshot = startedDuringWorkHours ? await findNodeSnapshot() : null;

  if (snapshot) {
    console.log(
      `starting during the work hours (${workHours}), skipping the preloading script ; the ${snapshot.nodes.length} nodes of the snapshot will be loaded during idle time`
    );
    // not awaited on purpose : it runs in the background, in the gaps between
    // the requests the organ is answering
    runSnapshotPreloader(spinalAPIMiddleware, profileId, snapshot).catch(
      (err) => {
  ...
  try {
    await preloadingScript(spinalAPIMiddleware, profileId, scriptOptions);
  } catch (err) {
