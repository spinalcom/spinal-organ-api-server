...
module.exports = function (logger, app, spinalAPIMiddleware) {
    ...
    app.delete('/api/v1/organization/:parentOrganizationId/:organizationIdToBeRemoved', (0, express_zod_safe_1.default)({
        params: zod_1.z.strictObject({
            parentOrganizationId: zod_1.z.coerce.number().positive(),
            organizationIdToBeRemoved: zod_1.z.coerce.number().positive(),
        }),
    }), async (req, res) => {
        try {
            ...
            try {
                const organizationParent = await (0, loadAndValidateNode_1.loadAndValidateNode)(spinalAPIMiddleware, parentOrganizationId, profileId);
                if (![
                    spinal_model_user_service_1.SPINAL_ORGANIZATION_CONTEXT_TYPE,
                    spinal_model_user_service_1.SPINAL_ORGANIZATION_TYPE,
                ...
                const organizationToBeRemoved = await (0, loadAndValidateNode_1.loadAndValidateNode)(spinalAPIMiddleware, organizationIdToBeRemoved, profileId, spinal_model_user_service_1.SPINAL_ORGANIZATION_TYPE);
                await (0, spinal_model_user_service_1.removeOrganization)(organizationToBeRemoved, organizationParent);
                res.status(204).send();
            }
            catch (error) {
                throw {
