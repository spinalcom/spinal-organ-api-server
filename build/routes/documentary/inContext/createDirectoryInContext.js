...
module.exports = function (logger, app, spinalAPIMiddleware) {
    ...
    app.post("/api/v1/documentary/create_directory/:contextId/:parentId", async (req, res, next) => {
        try {
            ...
            if (isNaN(contextId))
                return res.status(400).send({ message: "Invalid contextId parameter" });
            if (isNaN(parentId))
                return res.status(400).send({ message: "Invalid parentId parameter" });
...
//# sourceMappingURL=createDirectoryInContext.js.map