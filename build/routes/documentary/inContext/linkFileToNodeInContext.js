...
module.exports = function (logger, app, spinalAPIMiddleware) {
    ...
    app.post("/api/v1/documentary/file/link_to_node/:contextId/:parentId", async (req, res, next) => {
        try {
            ...
            const parentNode = await spinalAPIMiddleware.load(parentDynamicId, profileId);
            if (!parentNode)
                return res.status(404).send({ message: `No parent node found with id ${parentDynamicId}` });
            const promises = filesIds.map(async (fileDynamicId) => {
                ...
                return spinal_env_viewer_plugin_documentation_service_1.serviceDocumentation.addFileToNodeInContext(contextNode, parentNode, fileNode).catch((error) => {
