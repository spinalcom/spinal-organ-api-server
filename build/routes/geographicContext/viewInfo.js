...
function default_1(logger, app, spinalAPIMiddleware) {
    ...
    app.post('/api/v1/geographicContext/viewInfo', async (req, res) => {
        try {
            ...
            const result = await (0, viewInfo_func_1.viewInfo_func)(spinalAPIMiddleware, profilId, options);
            ...
            logger?.error?.(`Error in viewInfo route: ${error instanceof Error ? error.message : String(error)}`);
