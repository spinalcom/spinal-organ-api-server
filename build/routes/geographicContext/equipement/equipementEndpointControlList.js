    ...
    app.get("/api/v1/equipement/:id/control_endpoint_list", async (req, res, next) => {
        try {
            ...
            if (equipement.getType().get() === "BIMObject") {
                var profils = await spinal_env_viewer_graph_service_1.SpinalGraphService.getChildren(equipement.getId().get(), [spinal_env_viewer_plugin_control_endpoint_service_1.spinalControlPointService.ROOM_TO_CONTROL_GROUP]);
