...
module.exports = function (logger, app, spinalAPIMiddleware) {
    ...
    app.post("/api/v1/room/inventory_multiple", async (req, res, next) => {
        try {
            ...
            const promises = ids.map(id => (0, getRoomInventory_1.getRoomInventory)(spinalAPIMiddleware, profileId, id));
            ...
            const finalResults = settledResults.map((result, index) => {
                if (result.status === 'fulfilled') {
                    ...
                    return {
                        dynamicId: ids[index],
                        error: result.reason?.message || result.reason || "Failed to get Inventory"
                    };
                }
            });
...
//# sourceMappingURL=roomInventoryMultiple.js.map