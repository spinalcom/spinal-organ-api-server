...
module.exports = function (logger, app, spinalAPIMiddleware) {
    ...
    app.post('/api/v1/ticket/find_entity_multiple', async (req, res, next) => {
        try {
            ...
            const promises = ids.map((id) => (0, getTicketEntityInfo_1.getTicketEntityInfo)(spinalAPIMiddleware, profileId, id));
            ...
            const finalResults = settledResults.map((result, index) => {
                if (result.status === 'fulfilled') {
                    ...
                    return {
                        dynamicId: ids[index],
                        error: result.reason?.message || result.reason || 'Failed to get Entity',
                    };
                }
            });
...
//# sourceMappingURL=ticketFindEntityMultiple.js.map