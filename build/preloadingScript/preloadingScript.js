...
const getTicketDetails_1 = require("src/utilities/workflow/getTicketDetails");
...
async function processTicketList(spinalAPIMiddleware, profileId, server_ids) {
    ...
    const proms = uniqueTicketIds.map(async (server_id) => {
        if (server_id)
            await (0, getTicketDetails_1.getTicketDetails)(spinalAPIMiddleware, profileId, server_id, false);
    });
